...
export default function ClientDashboard() {
    ...
    useEffect(() => {
        api.get("/client/dashboard").then((r) => setData(r.data)).catch(() => {});
    }, []);
            ...
            {data.next_invoice ? (
                ...
                <div className="mb-8 rounded-2xl border border-emerald-200 bg-emerald-50/40 p-6 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="text-sm text-zinc-600">No pending invoices. Sit back — we're cooking up your next deliverable.</div>
                    </div>
                </div>
            )}
            ...
            <div className="grid lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <Section
                        ...
                        link={{ to: "/client/updates", label: "View all" }}
                    >
                        {(data.updates || []).length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {data.updates.map((u2) => (
                                    <div key={u2.id} className="border border-zinc-200 rounded-xl p-4 hover:border-zinc-300 transition">
                                        <div className="flex items-start justify-between gap-3">
                                            <div className="min-w-0">
                                                <div className="flex items-center gap-2">
                                                    <Pill status={u2.category || "update"} />
                                                    <span className="text-xs text-zinc-400">
                                                        {new Date(u2.created_at).toLocaleDateString()}
                                                    </span>
                                                </div>
                                                <div className="font-semibold text-zinc-900 mt-1.5">{u2.title}</div>
                                                <div className="text-sm text-zinc-600 mt-1 line-clamp-2">{u2.body}</div>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                ...
                <div className="space-y-6">
                    <Section title="Working hours" hint="Eastern Standard Time">
                        <div className="border border-zinc-200 rounded-xl overflow-hidden">
                            {[
                                ...
                                <div key={d} className="flex justify-between px-4 py-3 border-b border-zinc-100 last:border-0 text-sm">
                    ...
                    <Section title="Quick links">
                        <div className="space-y-2">
                            <QuickLink to="/client/invoices" label="Billing & invoices" />
                            <QuickLink to="/client/addons" label="Browse add-ons" />
                            <QuickLink to="/client/support" label="Talk to support" />
                            <QuickLink to="/client/profile" label="Update profile" />
                        </div>
                    </Section>
                </div>
            </div>
...
function Section({ title, hint, link, children }) {
    return (
        <div>
            <div className="flex items-end justify-between mb-3">
                ...
                {link && (
                    <Link to={link.to} className="text-xs font-semibold text-[#F77418] hover:underline">
                        {link.label} →
                    </Link>
                )}
            </div>
