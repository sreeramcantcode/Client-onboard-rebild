...
export default function AdminTicketsPage() {
  ...
  const load = async () => {
    const r = await api.get<Ticket[]>("/admin/tickets");
    setTickets(r.data || []);
    if (open) {
  ...
  const setStatus = async (status: string) => {
    if (!open) return;
    await api.patch(`/admin/tickets/${open.id}/status`, { status });
    load();
  };
  ...
  return (
    <div className="p-6 md:p-10">
      <PageHeader
        eyebrow="Support"
        title="Tickets"
        subtitle="Reply to client questions, mark resolved, keep the love flowing."
      />
      {tickets.length === 0 ? (
        <EmptyState
          icon={TicketIcon}
          title="No tickets"
          description="When clients open a ticket, it'll appear here."
        />
      ) : (
            ...
            {tickets.map((t) => (
              ...
              >
                ...
                <div className="text-xs text-zinc-500 mt-1">
                  {t.client_name} · {new Date(t.updated_at).toLocaleString()}
                </div>
              </button>
            ))}
          </div>
              ...
              <>
                <div className="border-b border-zinc-100 pb-4 mb-4 flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="font-display font-bold text-xl text-zinc-900 mt-2">
                      {open.subject}
                    </div>
                      ...
                      <div key={m.id} className={`flex ${mine ? "justify-end" : ""}`}>
                        <div
                          className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm ${
                            mine ? "bg-[#F77418] text-black" : "bg-zinc-100 text-zinc-900"
                          }`}
                        >
                          <div
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  />
                  <PrimaryButton onClick={send} data-testid="admin-ticket-send">
