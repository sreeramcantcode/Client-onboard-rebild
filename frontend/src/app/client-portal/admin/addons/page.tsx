...
export default function AdminAddonsPage() {
  ...
  const [form, setForm] = useState<{ name: string; description: string; price: number | string; icon: string }>({
    name: "",
    description: "",
    price: 0,
    icon: "Sparkles",
  ...
  const load = async () => {
    const [a, r] = await Promise.all([
      ...
      api.get<AddOnRequest[]>("/admin/addons/requests"),
    ]);
    setAddons(a.data || []);
    setRequests(r.data || []);
  };
  ...
  const submit = async () => {
    ...
    setCreating(false);
    setForm({ name: "", description: "", price: 0, icon: "Sparkles" });
    load();
  };
  ...
  const updateRequest = async (id: string, status: "open" | "pending" | "resolved" | "closed") => {
    await api.patch(`/admin/addons/requests/${id}`, { status });
    load();
  };
  ...
  return (
    <div className="p-6 md:p-10">
      <PageHeader
        ...
        action={
          <PrimaryButton onClick={() => setCreating(true)} data-testid="new-addon-button">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-10">
        {addons.length === 0 ? (
          <div className="col-span-full">
            <EmptyState icon={Plus} title="No add-ons yet" />
          </div>
        ) : (
          addons.map((a) => (
            <div key={a.id} className="border border-zinc-200 rounded-xl p-4 group">
              <div className="flex items-start justify-between">
                ...
                <button
                  onClick={() => remove(a.id)}
                  className="p-1.5 rounded-md hover:bg-red-50 text-red-500 opacity-0 group-hover:opacity-100 transition"
                >
      ...
      {requests.length === 0 ? (
        <div className="text-sm text-zinc-500 border border-dashed border-zinc-200 rounded-xl p-6 text-center">
                  ...
                  <td className="px-4 py-3 text-right">
                    {r.status === "pending" && (
                      <div className="flex justify-end gap-2">
                        ...
                        <button
                          onClick={() => updateRequest(r.id, "closed")}
      ...
      <Modal open={creating} onClose={() => setCreating(false)} title="New add-on">
        <div className="space-y-4">
          ...
          <div>
            <div className="text-xs font-semibold text-zinc-700 mb-1.5">Price</div>
            <Input
              type="number"
              step="0.01"
              value={form.price}
          ...
          <div>
            <div className="text-xs font-semibold text-zinc-700 mb-1.5">Lucide icon</div>
            <Input
              value={form.icon}
              onChange={(e) => setForm({ ...form, icon: e.target.value })}
            />
          </div>
          <PrimaryButton onClick={submit} disabled={!form.name} data-testid="create-addon-submit">
