...
export default function AdminInvoicesPage() {
  ...
  const total =
    form.items.reduce((s, it) => s + (Number(it.qty) || 0) * (Number(it.unit_price) || 0), 0) +
    Number(form.tax || 0);
    ...
    <div className="p-6 md:p-10">
      ...
      {invoices.length === 0 ? (
        <EmptyState
          ...
          description={
            clients.length === 0
              ? "Create a client first, then issue an invoice."
              : "Issue your first invoice to a client."
          }
        />
      ) : (
      ...
      <Modal open={creating} onClose={() => setCreating(false)} title="Create invoice" size="lg">
        <div className="space-y-4">
          ...
          <PrimaryButton
            onClick={submit}
            disabled={!form.client_id || !form.items.some((it) => it.description)}
