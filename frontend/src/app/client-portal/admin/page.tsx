...
import { Users, Receipt, DollarSign, Ticket, Plus } from "lucide-react";
...
interface Stats {
  ...
  recent_invoices: { id: string; number: string; client_name: string; total: number; status: string }[];
  recent_tickets: { id: string; subject: string; client_name: string; status: string }[];
}
  ...
  return (
    <div className="p-6 md:p-10">
      <PageHeader
        ...
        action={
          <Link
            href={`${PORTAL_BASE}/admin/clients`}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        <StatCard
          ...
          hint={`${stats.active_clients} active`}
          icon={Users}
          accent
        />
        <StatCard
        ...
        <StatCard
          ...
          hint={`${stats.addon_pending} add-on requests`}
          icon={Ticket}
        />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 border border-zinc-200 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link
              href={`${PORTAL_BASE}/admin/invoices`}
              className="text-xs font-semibold text-[#F77418] hover:underline"
            >
              All invoices →
            </Link>
          </div>
          {stats.recent_invoices?.length === 0 ? (
            ...
            <div className="space-y-2">
              {stats.recent_invoices.map((inv) => (
                <div
                  key={inv.id}
                  className="flex items-center justify-between p-3 border border-zinc-100 rounded-lg hover:border-zinc-200 transition"
                >
        ...
        <div className="border border-zinc-200 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link
              href={`${PORTAL_BASE}/admin/tickets`}
              className="text-xs font-semibold text-[#F77418] hover:underline"
            >
              View →
            </Link>
